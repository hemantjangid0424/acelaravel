...
var CurrentPage = class {
  ...
  set(page22, {
    ...
    return this.resolve(page22.component).then((component2) => {
      ...
      replace = replace || isSameUrlWithoutHash(hrefToUrl(page22.url), location);
...
function convertSequentialObjectsToArrays(value) {
  ...
  for (const key2 in value) {
    result[key2] = convertSequentialObjectsToArrays(value[key2]);
  }
  return result;
}
...
var useInfiniteScrollElementManager = (options) => {
  ...
  const restoreElements = () => {
    ...
    for (let index = 0; index < childNodes.length; index++) {
      ...
      const element = node;
      let elementPage;
      for (const [page22, range] of Object.entries(pageElementRange)) {
  ...
  const form = reactive({
    ...
    withPrecognition(...args2) {
      ...
      const validator = createValidator((client2) => {
        ...
        const transformedData = cloneDeep_default(transform(this.data()));
...
var Form = defineComponent({
  ...
  setup(props, { slots, attrs, expose }) {
    ...
    const onFormUpdate = (event) => {
      ...
      isDirty.value = event.type === "reset" ? false : !isEqual_default(getData(), formDataToObject(defaultData.value));
    };
